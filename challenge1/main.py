...
#     return x + np.random.normal(0, sigma, x.shape).astype('float32')

# def augment_scaling(x, sigma=0.1):
#     factor = np.random.normal(1.0, sigma, (x.shape[1],)).astype('float32')
#     return x * factor
...
# def apply_augmentations(window,strenght):
#     # window: (seq_len, features)
#     # probabilistic pipeline
#     if np.random.rand() < 0.5:
#         window = augment_jitter(window, sigma=0.02*strenght)
#     if np.random.rand() < 0.4:
#         window = augment_scaling(window, sigma=0.1*strenght)
#     if np.random.rand() < 0.3:
#         window = augment_time_shift(window, max_shift=5)
#     return window



# ---- stronger / richer augmentations ----
def augment_jitter(x, sigma=0.03):
    # gaussian noise per campione
    return x + np.random.normal(0, sigma, x.shape).astype('float32')

def augment_scaling(x, sigma=0.15):
    # scaling per feature
    factor = np.random.normal(1.0, sigma, (x.shape[1],)).astype('float32')
    return x * factor

def augment_time_shift(x, max_shift=8):
    shift = np.random.randint(-max_shift, max_shift+1)
    if shift == 0:
        return x
    return np.roll(x, shift, axis=0)
...
def channel_dropout(x, drop_prob=0.1):
    # setto a zero intere feature columns con probabilità drop_prob
    mask = (np.random.rand(x.shape[1]) > drop_prob).astype('float32')
    return x * mask[None, :]

def time_mask(x, max_mask_len=8):
    # mask temporale contigua (simile a SpecAugment)
    L = x.shape[0]
    m = np.random.randint(0, max_mask_len+1)
    if m == 0: return x
    start = np.random.randint(0, max(1, L-m))
    x[start:start+m, :] = 0.0
    return x

def mixup(a, b, alpha=0.2):
    # assume same shape, returns linear blend
    lam = np.random.beta(alpha, alpha)
    return a * lam + b * (1 - lam), lam

# on-the-fly pipeline
def apply_augmentations_on_the_fly(window, strength=1.0):
    # strength in [0.0, 1.5] roughly, scales magnitudes
    w = window.copy()
    # probabilities increase with strength
    p_jitter = 0.6 if strength>0.6 else 0.45 + 0.3*strength
    p_scale  = 0.5 if strength>0.6 else 0.3 + 0.25*strength
    p_shift  = 0.4 if strength>0.6 else 0.2 + 0.2*strength
    p_chan_drop = 0.2 * strength
    p_time_mask = 0.25 * strength

    if np.random.rand() < p_jitter:
        w = augment_jitter(w, sigma=0.01 + 0.04*strength)   # 0.01..0.05
    if np.random.rand() < p_scale:
        w = augment_scaling(w, sigma=0.02 + 0.12*strength)  # 0.02..0.14
    if np.random.rand() < p_shift:
        w = augment_time_shift(w, max_shift=int(2 + 6*strength))  # 2..8
    if np.random.rand() < p_chan_drop:
        w = channel_dropout(w, drop_prob=min(0.5, 0.08 * (1+strength)))
    if np.random.rand() < p_time_mask:
        w = time_mask(w, max_mask_len=int(3 + 10*strength))
    # occasional small crop/resize to simulate different lengths (safe if label invariant)
    if np.random.rand() < 0.12:
        w = random_crop(w, target_len=window.shape[0])
    return w


from torch.utils.data import Dataset
class TimeSeriesDataset(Dataset):
    def __init__(self, X, y, mode="train", augment_strength=1.0):
    ...
    def __getitem__(self, idx):
        x = self.X[idx].copy()
        y = int(self.y[idx])
        if self.mode == "train" and self.augment_strength > 0.0:
            x = apply_augmentations_on_the_fly(x, strength=self.augment_strength)
...
if __name__ == "__main__":
    ...
    num_of_augmentations = 0  
    augmentatino_strength = 0
    ...
    # class_counts = submission_df['label'].value_counts().sort_index()
    ...
    cw = compute_class_weight('balanced', classes=np.array([0,1,2]), y=y_train['label'])
    class_weights = torch.tensor(cw, dtype=torch.float32).to(device)

    
    #criterion = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=0.05)
    #criterion = nn.CrossEntropyLoss(weight=torch.tensor([1,1,1],dtype=torch.float32).to(device), label_smoothing=0.05)

    # Set up loss function
    #criterion = nn.CrossEntropyLoss(weight=torch.tensor(class_weights.values, dtype=torch.float32).to(device))
    ...
    # criterion = FocalLoss(gamma=2.0, weight=torch.tensor(class_weights,dtype=torch.float32).to(device))
    ...
    def build_sequences(df, window=200, stride=200,train=False):
        ...
        for id in df['sample_index'].unique():
            ...
            while idx + window <= len(temp):
                ...
                if train:
                    for _ in range(num_of_augmentations*(label)):
                        # Apply augmentations only for training data
                        augmented_window =temp[idx:idx + window] #apply_augmentations(temp[idx:idx + window],0)
                        dataset.append(augmented_window)
                        labels.append(label)
    ...
    x_train_final,y_train_final = build_sequences(df_train,window=WINDOW_SIZE,stride=STRIDE,train=True)
    ...
    train_ds = TimeSeriesDataset(x_train_final, y_train_final, mode="train", augment_strength=augmentatino_strength)
    val_ds = TimeSeriesDataset(x_val, y_val, mode="val")
    ...
    # Define optimizer with L2 regularization
    optimizer = torch.optim.AdamW(rnn_model.parameters(), lr=LEARNING_RATE, weight_decay=L2_LAMBDA)
    scheduler = scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=4)
    ...
    torch.save(best_model.state_dict(), "models/best_"+experiment_name+'_model.pt')
